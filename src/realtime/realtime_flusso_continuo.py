...
# Oggetto sentinella dedicato per segnalare "fine cattura" nella coda dei
# frame. Usiamo un oggetto unico invece di None per evitare qualunque
# ambiguità con un frame "vuoto" o con un timeout della coda.
_SENTINEL = object()
...
def pose_worker_and_classify(frame_queue, inferencer, clf, width, height,
                              shared_state, result_holder, pose_batch_size=BATCH_SIZE):
    keypoints_list = []
    pending_frames = []
    capture_ended = False
    ...
    while True:
        ...
        # Elabora un mini-batch appena ne accumuliamo abbastanza, oppure
        # subito se la cattura e' finita e resta un residuo piu' piccolo
    ...
    # Salva subito prima di classificare: cosi' i
    ...
    print(f"salvato in {output_pkl}")
   
    features = extract_features(keypoints, effective_fps)
    if features is None:
        print("Troppi frame non validi Impossibile classificare.")
...
def main():
    ...
    worker_thread = threading.Thread(
        ...
        args=(frame_queue, inferencer, clf, width, height, shared_state, result_holder),
        daemon=True,
    )
    worker_thread.start()
    ...
    while True:
        ...
        if elapsed <= BUFFER_SECONDS:
            ...
            print(f"\nCattura completata: {n_captured} frame in "
                  ...
                  f"fps: {shared_state['effective_fps']:.2f}")
